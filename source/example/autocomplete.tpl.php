...
<?php require(__DOCROOT__ . __EXAMPLES__ . '/includes/header.inc.php'); ?>
	...
	<div class="instructions">
		...
		<p>Frequently, you have a need to allow the user to type in a value into a textbox,
		but there are some pre-set values that you'd like the user to pick from. One way
		to solve this problem is to use a <strong>QListBox</strong> control; that said, when the
		list is really large - or you want to allow the user to pick values not on the list
		- you need true auto-complete functionality.</p>

		<p>QCubed comes with two implementations of auto-suggest, both based on jQuery:
		client-side and server-side. You can see them in action in the example below.</p>
		
		<p>With <strong>QJavaScriptAutoCompleteTextBox</strong>, you define the full set of options
		...
		paying a small cost every time the user types something.</p>
		
		<p>In contrast, <strong>QAjaxAutoCompleteTextBox</strong> allows you to define a true Ajax-style
		...
		time with the JavaScript-based implementation.</p>
	</div>

	<p>Try typing the letter "J" into the textboxes below:</p>
	<p>Client side (QJavaScriptAutoCompleteTextBox): <?php $this->txtClientSide->Render(); ?></p>
	<p>Server side (QAjaxAutoCompleteTextBox): <?php $this->txtServerSide->Render(); ?></p>
